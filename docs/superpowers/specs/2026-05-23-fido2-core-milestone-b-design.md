...
| `MdsBlobProvider` | 내부 교체 — webauthn4j `FidoMDS3MetadataBLOBProvider` delegate → Spring `RestClient` fetch + `fido2.mds.MetadataBlob.parse()`. lifecycle·캐싱·`/_diag` 노출은 유지 |
| `MdsTrustAnchorRepositoryConfig` | 제거 — webauthn4j `TrustAnchorRepository` 전용 wiring. 자체 `MdsTrustAnchorSource` 빈 wiring으로 대체 |
