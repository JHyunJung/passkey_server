...
자체 FIDO2 코어 (`com.crosscert.passkey.fido2`) ceremony 검증 코어 — 인증 + 전체 등록(strict/non-strict) (CBOR/COSE/attestation 자체 구현, WebAuthn L3). Milestone B Phase 4에서 webauthn4j 완전 제거.
...
| 2026-05-23 | 자체 FIDO2 코어 (Milestone B Phase 3) | webauthn4j 제거 1단계 — FIDO MDS3 인프라 자체 구현 + 추가 attestation 포맷. 신규 `fido2.mds` 패키지: `MetadataBlob`(BLOB JWS 자체 파싱 — 서명검증 nimbus-jose-jwt, x5c 체인 JCA `CertPathValidator`, payload(JSON) 해석 자체 구현, 손상 entry 스킵), `MetadataEntry`/`StatusReport`(FIDO 인증 상태 — critical 5종 분류), `MdsTrustAnchorSource`(AAGUID별 trust anchor 조회). 신규 `fido2.attestation.AttestationCertPathValidator`(JCA PKIX 래퍼). `AttestationVerifier.verify()`에 nullable `MdsTrustAnchorSource` 파라미터 추가 — non-null이면 strict(cert chain trust anchor 검증 + revoked 거부), null이면 non-strict. `PackedAttestationVerifier`의 x5c full 경로에 trust anchor 검증 추가. attestation verifier 추가: `apple`(nonce 확장 SHA-256(authData‖clientDataHash) 일치 + `PublicKey.equals()` 공개키 비교), `android-key`(서명 + Android Key Attestation 확장 `attestationChallenge` ↔ clientDataHash 일치 + `PublicKey.equals()`). ASN.1 파싱은 자체 `DerUtil`(BouncyCastle production 의존 안 함). `MdsBlobProvider`에 자체 `MetadataBlob` 파싱을 병행 추가(RestClient fetch), webauthn4j `FidoMDS3MetadataBLOBProvider` delegate는 strict 경로용으로 잔존. `MdsConfig`가 `MdsTrustAnchorSourceHolder` 빈 노출. `Fido2VerificationException.FailureReason`에 `TRUST_PATH_INVALID`/`MDS_TRUST_FAILED`/`AUTHENTICATOR_REVOKED` 추가. ArchUnit Rule 7 주석에 nimbus-jose-jwt 허용 명시. strict 경로(`RegistrationService.verifyWithWebauthn4j`)는 Phase 3에서 미전환 — Phase 4 대상. `webauthn4j-core`/`webauthn4j-metadata` 의존성 잔존. 설계: `docs/superpowers/specs/2026-05-23-fido2-core-milestone-b-design.md`, plan: `docs/superpowers/plans/2026-05-23-fido2-core-milestone-b-phase3.md`. |
| 2026-05-23 | 자체 FIDO2 코어 (Milestone B Phase 4) | webauthn4j 완전 제거. attestation 7포맷(none / packed / apple / android-key / android-safetynet / fido-u2f / tpm) + strict/non-strict 단일 경로 모두 자체 코어(`com.crosscert.passkey.fido2`) 단독으로 동작. `WebAuthnConfig` / `MdsTrustAnchorRepositoryConfig` 삭제, `MdsBlobProvider` 자체 모델 단독화(webauthn4j `FidoMDS3MetadataBLOBProvider` delegate 제거), `RegistrationService.verifyWithWebauthn4j` 삭제 → `verifyWithCore` 단일화. `webauthn4j-core` / `webauthn4j-metadata` build.gradle 의존성 제거. ArchUnit Rule 8(`no_webauthn4j_in_production` — production 소스에서 `com.webauthn4j` import 금지) 추가. 차등 테스트(`AssertionDifferentialTest` / `RegistrationDifferentialTest`) → BouncyCastle 자체 빌드 골든 벡터 기반으로 전환. 모든 attestation verifier의 unknown alg 분기를 `UNSUPPORTED_ALGORITHM`으로 통일(`ATTESTATION_INVALID` → `UNSUPPORTED_ALGORITHM`). `TRUST_PATH_INVALID`는 `ATTESTATION_INVALID`(400) → `MDS_TRUST_FAILED`(403)로 매핑되어 strict 모드 cert-chain 실패의 의미가 더 명확해짐. |
| 2026-05-24 | Admin 사용자 360° 뷰 | 테넌트 상세 Users 탭에서 end-user 선택 시 별도 상세 화면(`/users/:tenantUserId`) 열림 — Credentials/Sessions 탭(paging) + AAGUID 라벨(MdsBlobProvider 기반, 미등재 인증기는 "미등록" 배지) + 개별 credential revoke / session revoke + "모두 로그아웃" 액션. `AdminEndUserController`(`GET /api/v1/admin/tenants/{tenantId}/users`, `.../users/{tenantUserId}`), `TenantUserRepository.findByTenantIdWithSearch`(N+1 없이 활성 passkey count 집계), `AdminEndUserControllerSliceTest`/`AdminEndUserIntegrationTest`. **BREAKING**: `AdminEndUserDetailView` 신규(기존 `AdminUserSessionController.EndUserDetailView`와 다름 — credential 조회 추가). 설계: `docs/superpowers/specs/2026-05-24-admin-user-360-view-design.md`. |
