...
| env | 값 | 효과 |
|-----|----|------|
| `PASSKEY_ADMIN_ENABLED` | `true` | Admin REST API + `APP_ADMIN` 데이터소스 활성화 — 콘솔이 동작하려면 **필수** |
...
> **JWT 알고리즘과 무관**: admin 콘솔은 인증에 Spring Session 쿠키를 쓰므로 `PASSKEY_JWT_ALGORITHM`(HS256/RS256) 선택은 콘솔 배포에 영향을 주지 않습니다. JWT 설정은 RP-facing 토큰 발급 전용 — RP 백엔드 배포 시 `docs/deployment.md` "JWT 서명 알고리즘" 참조.
